...
// Personal "watched / done" marks live under  watched:{email}  -> JSON string[] of session ids.
// Private to each user (mirrors interest.ts), but tracks "已看 / done" rather than "want to watch".

const unauthorized = () => Response.json({ error: "unauthorized" }, { status: 401 });

// GET /api/watched -> { ids: string[] }
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  const user = await getUser(request, env);
  if (!user) return unauthorized();
  const raw = await env.NOTES_KV.get(`watched:${user.email}`);
  return Response.json({ ids: raw ? JSON.parse(raw) : [] });
};

// POST /api/watched  body: { id: string, on?: boolean }  (on omitted = toggle)
